#!/usr/bin/env bash
# Stack3 真机：发力矩执行。务必先跑 run_live_stack3_readonly.sh 并确认 action 语义。
set -euo pipefail
...
RUN_DIR="$PI0/diagnostics/live_stack3_exec_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RUN_DIR"
...
echo "EXECUTE live: torque WILL enable. E-stop ready."
echo "If action0 looks like small deltas while state is ~tens of degrees, STOP — relative actions need conversion."
